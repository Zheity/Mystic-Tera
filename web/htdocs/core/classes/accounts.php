...
class Accounts
{
	...
	{
		$allowed = array('AccountId', 'Username', 'Email');
	...
	{
		$query = $this->db->prepare("SELECT COUNT(`AccountId`) FROM `accounts` WHERE `Username`= ?");
	...
	{
		$query = $this->db->prepare("SELECT COUNT(`AccountId`) FROM `accounts` WHERE `Username`= ? AND `AccessLevel` = ?");
		$query->bindValue(1, $username);
		$query->bindValue(2, 1);
	...
	{
		$query = $this->db->prepare("SELECT `Password`, `AccountId` FROM `accounts` WHERE `Username` = ?");
		$query->bindValue(1, $username);
		try
		{
			$query->execute();
	...
	public function register($username, $password, $email)
	{
		...
		$query 	= $this->db->prepare("INSERT INTO `accounts` (`Username`, `Password`, `Email`, `AccessLevel`, `Membership`, `isGM`, `LastOnlineUtc`, `Coins`, `Ip`, `EmailVerify`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ");
	...
	{
		$query = $this->db->prepare("SELECT COUNT(`AccountId`) FROM `accounts` WHERE `Email` = ? AND `EmailVerify` = ? AND `AccessLevel` = ?");
	...
	public function password_recovery($email, $password_recovery)
	{
		...
		else
		{
			...
			try
			{
				...
				if($rows == 1)
				{
					$username = $this->fetch_info('Username', 'Email', $email); //get username for the use in the email.
